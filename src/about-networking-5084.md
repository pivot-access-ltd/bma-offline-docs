...
MAAS provides an [internal proxy](https://maas.io/docs/how-to-manage-proxies#heading--internal-proxy-maas-proxy), which is an HTTP caching proxy server that is availble to all hosts residing in any subnet managed by MAAS.  In addition, MAAS allows you to define an [external proxy](https://maas.io/docs/how-to-manage-proxies#heading--configure-proxy) if desired.
