...
A fabric is simply a way of linking [VLANs](/t/maas-concepts-and-terms-reference/nnnn#heading--vlans) (Virtual LANs) together.  If you're familiar with a VLAN, you know that it's designed to limit network traffic to specific ports (e.g., on a [switch](/t/maas-concepts-and-terms-reference/nnnn#heading--switch)) or by evaluating labels called "tags" (unrelated to MAAS tags).  By definition, this would mean that two VLANs can't communicate with each other -- it would defeat the purpose of the VLAN -- unless you implement some extraordinary measures.
...
You can learn more about fabrics in the [Concepts and terms](/t/maas-concepts-and-terms-reference/nnnn#heading--fabrics) section of this documentation.
</details>
...
5. The rack controller tasked with providing DHCP handles assigning an IP address to the machine via the [DORA](/t/maas-concepts-and-terms-reference/nnnn#heading--dhcp) sequence (Discover, Offer, Request, Acknowledge).  **Note** that this rack controller doesn't have to be the same one that powers on the machine.
