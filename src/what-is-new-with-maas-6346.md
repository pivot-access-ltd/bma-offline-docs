...
What is ansible:
Red hat offers up this explanation.
The Ansible docs offer this.
<Reword> Fancy Yaml that does automation
A user should probably have a solid grasp of the standard terminology in relation to ansible:
Playbook / plays
Hosts and groups
What have we done?
A playbook that will install and configure a MAAS region on a targeted host    Jack
Running the playbook on hosts with a MAAS region will upgrade it.
User story 1
A playbook that will install and configure a MAAS rack                Jack
(This has not landed yet, but should do shortly)
User story 2
...
MAAS Playbooks stored in this repo, or found through Ansible Galaxy (eventually).
