...
| 2. | `title`| Human-friendly descriptive version of the name, used within the web UI.|
| 3. | `description`| Brief outline of what the script does.|
| 4. | `tags`| List of tags associated with the script.|
| 5. | `type`| Either commissioning or testing.|
| 6. | `timeout`| Length of time before MAAS automatically fails and kills execution of the script. The time may be specified in seconds or using the HH:MM:SS format. |
| 7. | `destructive`| True or False, depending on whether the script will overwrite system data. You can't run destructive tests on a deployed machine. |
| 8. | `comment`| Describes changes made in this revision of the script.  A comment can be passed via the API when uploading the script.  MAAS doesn’t look at the script metadata for this field. |
| 9. | `hardware_type`| Defines the type of hardware the script configures or tests. If the script returns results, hardware types associate the results with specific hardware. The following types are valid: |
...
| 10. |   `parallel`| Enables scripts to be run in parallel and can be one of the following: |
...
| 11. | `parameters`| What [parameters](#heading--parameters) the script accepts. |
| 12. | `results`| What [results](#heading--results) the script will return. |
| 13. | `packages`| List of packages to be installed or extracted before running the script. Packages must be specified as a dictionary. For example, `packages: {apt: stress-ng}`, would ask `apt` to install stress-ng. Package sources can be any of the following: |
...
| 14. |   `for_hardware`| Specifies the hardware that must be on the machine for the script to run. May be a single string or list of strings of the following:
...
| 15. |   `may_reboot` |When True, indicates to MAAS that the script may reboot the machine. MAAS will allow up to 20 minutes between heartbeats while running a script with `may_reboot` set to True. |
| 16. | `recommission`| After all commissioning scripts have finished running rerun |
| 17. |   `script_type` | commissioning or test. Indicates whether the script should run during commissioning or hardware testing. |
...
| No. | Field | Description |
|----:|:------|:------------|
| 1. |   `type`: Every parameter must contain a type field, which describes what the parameter may accept and its default values. It may be one of the following: |
...
|| |   `storage`: all.|
|| |   `interface`: all.|
| 4. |   `required`: Whether or not user input is required. A value of false sets no default, and no user input will mean no parameters passed to the script. Defaults to `true`.|
| 5. |   `results`: What results the script will return on completion. You can only define this parameter within the embedded YAML of the script. Results may be a list of strings or a dictionary of dictionaries.|
